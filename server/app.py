...
def generate_book_endpoint():
    try:
        ...
        # Split chapter_titles string into a list of titles
        chapter_titles = [title.strip() for title in chapter_titles.split(',')]

        chapters_content = generate_book(
            writing_style, book_description, chapter_titles)
        title = generate_title(book_description)
        cover_image_path = create_cover_image(book_description)

        file_stream = io.BytesIO()
        create_doc(title, 'Author Name', chapters_content,
                   chapter_titles, cover_image_path, file_stream)
        file_stream.seek(0)
